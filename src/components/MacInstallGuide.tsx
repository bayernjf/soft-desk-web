...
export function MacInstallGuide({ open, onClose, theme }: MacInstallGuideProps) {
  ...
  useEffect(() => {
    if (open) track('mac_install_guide_view');
  }, [open]);
  ...
  return (
    ...
    >
      ...
      <div
        ...
        className={cn(
          'relative w-full max-w-md rounded-2xl border p-6 shadow-2xl animate-in',
          theme === 'light'
      ...
      >
        ...
        >
          ...
          <span>
            复制后打开「终端」app（Spotlight 搜索 <kbd className="px-1 py-0.5 rounded bg-slate-200 dark:bg-slate-700 text-slate-600 dark:text-slate-300 font-mono text-[10px]">terminal</kbd>），粘贴并回车。之后即可正常启动 SoftDesk。
          </span>
        </div>
        ...
        <div className="mt-5 flex gap-3">
          <button
            onClick={handleCopy}
            className="flex-1 inline-flex items-center justify-center gap-2 px-4 py-2.5 rounded-xl text-sm font-semibold transition-all bg-primary-500 hover:bg-primary-600 text-white shadow-glow-brand"
          >
